...
var taskTwbs        = require('./gulp-tasks/twbs.js');
...
//=======================================================
gulp.task('twbs', ['twbs:sass', 'twbs:moveJs', 'twbs:moveFonts']);

// Compile Bootstrap Sass
gulp.task('twbs:sass', function() {
  return taskTwbs.sass();
});

// Move Bootstrap js.
gulp.task('twbs:moveJs', function() {
  return taskTwbs.moveJs();
});

// Move Bootstrap fonts.
gulp.task('twbs:moveFonts', function() {
  return taskTwbs.moveFonts();
});

//=======================================================
// Clean all directories.
//=======================================================
gulp.task('clean', ['clean:css', 'clean:js', 'clean:styleguide', 'clean:twbs', 'clean:twbsFonts']);
...
gulp.task('clean:twbs', function () {
  return taskClean.twbs();
});

gulp.task('clean:twbsFonts', function () {
  return taskClean.twbsFonts();
});
...
gulp.task('watch:sass', function(callback) {
  runSequence(
    ['lint:sass', 'compile:sass', 'twbs:sass'],
    'concat',
    callback
  );
});
...
gulp.task('watch', function() {
  ...
  gulp.watch(
    './src/{global,layout,components,bootstrap}/**/*.scss',
    ['watch:sass']
  );
...
gulp.task('default', function(callback) {
  runSequence(
    'clean',
    ['lint', 'compile', 'compress', 'styleguide', 'twbs'],
    'concat',
    callback
  );
});
